...
`scripts/build-dependency-trees.mjs` writes `public/architect_flow_dependency_type_mapping-json/{version}.json` (flow deps the provider maps from `dependent_consumers.go`) and `public/dependency-tree-merged-json/{version}.json` (raw tree + flow deps + `overrides.json`), plus `index.json` and `latest.json` in each directory. **Scripts stay manual:** `addDependencies.genesyscloud_script` in `overrides.json` is curated from scripter documentation — not from `dependent_consumers.go`, which is flow-oriented shared Architect mapping. Invoked by `bootstrap-local-dev`, `download-provider-versions`, and `npm run build`. Pass `--latest=X.Y.Z` to build a single version.
...
`public/lab-packages/` is **generated** from `scripts/templates/cx-as-code-lab/`, **one zip per provider version** (same version list as `dependency-tree-merged-json/`). Each zip pins `version = "~> X.Y.Z"` in every lab `.tf` file that declares a provider constraint, writes `exportpipeline/main.tf` `exclude_filter_resources` from `spreadsheetTemplates.out` in `src/private-overrides.json` (skipping types in that file's `replace_with_datasource` block and `nonExportableResourceTypes`), and writes `exportall/main.tf` `exclude_filter_resources` from optional `labfiles.allout` (same skip rules; omit or leave empty for no exclusions). Lab package rebuilds track merged dependency trees (provider flow deps + overrides). Use the [Export builder](https://cxascode.github.io/exportbuilder/) to build export filters interactively.
